...
/* Reads and installs dependencies. No conflict checking. */
int Command_depends(apr_pool_t* p, const char* path) {
        FILE* in = NULL;
        bstring line = NULL;

        in = fopen(path, "r");
        check(in != NULL, "Failed to open deps file: %s", path);
...
int Command_fetch(apr_pool_t* p, const char* url, bool fetch_only) {
        ...
        if(apr_fnmatch(GIT_PAT, info.path, 0) == APR_SUCCESS) {
                ...
                check(!fetch_only, "No point in fetching a dep file.");
                ...
                check(rv == 0, "Failed to process deps: %s", url);
                ...
                check(rv == APR_SUCCESS,
                      "Failed to make directory %s", BUILD_DIR);
...
int Command_install(apr_pool_t* p,
        ...
        if(rc == 1) {
                ...
                log_info("Deps successfully installed: %s", url);
        } else {
